...
Download the latest PhotoOrganizer_v2.x.x.exe.
...
Launch the built-in update helper
Close the currently running application
Safely replace the application with the new version
Automatically restart the updated version
...
특히 기존 README에서 가장 크게 바뀐 건 `Installation` 부분이야. 이제 `PhotoOrganizer_Setup_v2.0.0.exe` 관련 내용은 전부 제거하는 게 맞아.

그리고 `Development`의 `Inno Setup`도 지금 최종 배포 방식에서는 사용하지 않으니까 뺐어.
그냥 완전히 복붙할수있게  너가 계속 준 Markdown방식으로 전문을 써서 보내줘
# Photo Organizer
